...
function check_os_ok {
    ...
    if [[ ! $LINUX_OS == "Ubuntu" ]]; then
        printf "** Error , ndogo-loop $NDOGO_LOOP_VERSION is only tested with Ubuntu OS at this time   **\n"
        exit 1
    fi 
    ...
    if [[ ! "$os_version_ok" == true ]]; then 
        printf "** Error , ndogo-loop $NDOGO_LOOP_VERSION is not tested with Ubuntu OS version [ %s ] at this time \n" "$LINUX_VERSION"
...
function do_k3s_install {
    ...
    su - $k8s_user -c "helm install --wait --timeout 300s ingress-nginx ingress-nginx \
                      ...
                      -f $NDOGO_LOOP_DIR/manifests/nginx-values.yaml" 
...
NDOGO_LOOP_DIR="$( cd $(dirname "$SCRIPTS_DIR")/ ; pwd )"
echo "NDOGO_LOOP_DIR=$NDOGO_LOOP_DIR"
COMMON_SCRIPTS_DIR="$NDOGO_LOOP_DIR/common-scripts"
echo "DBG> COMMON SCRIPTS_DIR X = $COMMON_SCRIPTS_DIR"

DEFAULT_K8S_DISTRO="k3s"   # default to k3s 
K8S_VERSION="" 
NDOGO_LOOP_VERSION="vNext"
